...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \DRAWERROR{3}{1}{1}{x}
    \DRAWERROR{3}{2}{1}{x}
    \node[script] at (P-1-1) {(c)};
    \node[script] at (S-3-1) {(d)};
    \node[script] at (P-0-3) {(a)};
    \node[script] at (S-2-3) {(b)};
  \end{tikzpicture}
  \hspace{1cm}
  \begin{tikzpicture}
    \DRAWPLANAR{4}
    ...
    \DRAWERROR{2}{3}{0}{x}
    \begin{pgfonlayer}{edges}
      \draw[synz] (N-0-1-0) -- (N-3-1-0);
      \draw[synx] (N-2-3-0) -- (N-2-0-0);
    \end{pgfonlayer}
    \node[script, above=.05cm of S-1-1] {(e)};
    \node[script, right=.05cm of P-2-3] {(f)};
  \end{tikzpicture}
