...
\begin{figure}[]
  ...
  \begin{tikzpicture}[on grid]
    ...
    \draw[lw1, -latex] (e) -- (f);
    \draw[lw1, -latex] (f) -- (g);
    \draw[lw1, -latex] (c) .. controls +(0.75,1) and +(-0.75,1) .. (c);
    \node at (1.5,-1) {(a)};
    ...
    \begin{scope}[shift={(6,-1)}]
      ...
      \draw[lw1, -latex] (g2) .. controls +(0.75,1) and +(-0.75,1) .. (g2);
    \end{scope}
  \end{tikzpicture}
  \caption{<caption>}
  \label{<label>}
...
To find the complexity of a sequence of \codefunc{Union} and \codefunc{Find} operators with both rules applied, we use a version of Ackermann's function $A(k,x)$ \cite{ackermann1928hilbertschen, kozen1992design}, which is very fast growing, and its inverse slow-growing $a(n)$. 
\begin{align}
  \nonumber A(0,x) &= x + 1\\
  A(k+1,x) &= A^x(k,x) \\
