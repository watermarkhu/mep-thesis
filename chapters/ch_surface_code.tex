...
A simple implementation of the instructions would be to store for each set $\bound(C_i)$ a list $l_i$ of all the vertices it contains, and a pointer to $l_i$ at all vertices $v\in l_i$. After a call to $\codefunc{Union}(\bound(C_i), \bound(C_j))$, all vertices of $l_j$ are appended to $l_i$, and the pointers of all vertices $v\in l_j$ are updated. This takes $\m{O}(m)$ operations, where $m=|l_j|$ the number of elements. On a lattice of $n$ vertices, there may be a maximum of $n-1$ calls to \codefunc{Union}. As the $i$\textsuperscript{th} call involves sets $l_i|=1$ and $l_i|=i$, the number of updates is 
\begin{equation}
  \sum_1^{n-1} i = \m{O}(n^2),
\end{equation}
where each \codefunc{Union} operation has on average $\m{O}(n)$. 

\subsubsection{Union by weight}
Suppose that east list $l_i$ additionally stores the length of the list itself, which is easily maintained, we can choose to always append the smaller list to the larger list. 

\subsubsection{Disjoint-set trees}

The cluster is stored as a tree, whose vertices each stores a parent pointer, and the root of the forest is an indexer of the cluster. A cluster may consist of a single vertex. However, if that vertex has a parent, the vertex is part of whatever cluster is identified by following the chain of parents upward in the tree until the root vertex, which is done by $\codefunc{Find}(v)$. In the case of a merge between clusters $C_i, C_j$, $\codefunc{Union}(C_i, C_j)$ combines the trees by making one tree a child of another, such that there is only one single root vertex that is the representative element. Note that parent pointers in the Union-Find data structure are \emph{not} equivalent to edges $e\in E$. The edge set $E$ with its erasure subset $\m{E}\subseteq E$ and subsequently cluster $C\subseteq \m{E}$ and forest $F_C\subseteq C$ are related to physical qubits and the lattice structure of the surface code, whereas edges of the tree $C$ exists to point towards the representative element at the root.
