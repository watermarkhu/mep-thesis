...
\begin{center}
    \begin{tikzcd}[row sep={0.65cm,between origins}]
      & \qw & \ctrl{2} & \qw & \qw &\\
      & \qw & \qw & \ctrl{1} & \qw &\\
      \lstick{$\ket{0}$} & \qw & \targ{} & \targ{} & \qw & \meter{Z}
    \end{tikzcd}
\end{center}
...
\paragraph{The depolarizing channel}
In the \emph{depolarizing} channel, the afflicted quantum state is replaced by a complete mixed state with probability $p_D$. Let the completely mixed state be written as
...
\end{align}
which can be interpreted as the state $\rho$ is left untouched with probability $p^*_D =\frac{3}{4}p_D$, and each Pauli gate is applied to it with probability $1-p^*_D$.

\paragraph{The erasure channel}
In the \emph{erasure} channel, a qubit is completely erased or lost from the system. Such a loss can be detected and the missing qubit is then replaced by a qubit in the totally mixed state (equation \ref{qec:eq:mixstate}). The erasure channel can therefore be seen as the depolarizing channel with the extra property that it can be detected which qubits suffer the error.
