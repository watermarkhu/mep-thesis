...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{A comparison between the matching weight and averaged running time between the various implementations of the Union-Find decoder listed in Table \ref{tab:ufndfwug}, and the Minimum-Weight Perfect Matching (MWPM) decoder. The specific data is taken from the Monte Carlo simulations of the decoding success rate of each of the decoders on a toric code for independent noise with $p_X = 0.098$. The x-axes scale proportional to the increase in the number of qubits $n\propto L^2$. Similar comparisons of (c) and (d) for simulations on the planar code and with phenomenological noise are included in Figures \ref{fig:mwcomp_uf} and \ref{fig:tcomp_uf}.}
