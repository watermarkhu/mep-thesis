...
\begin{algorithm}[htb]
  ...
  \BlankLine
  \caption{Dynamic-Forest Bucket Union-Find decoder}\label{algo:dfbuf}
\end{algorithm}

\subsection{Decoder labelling}
With the application of weighted growth through bucket sort and the maintenance of a dynamic forest, we now have several implementations of the Union-Find decoder. From this point on, we refer to Algorithm \ref{algo:uf} as the \emph{Static-Forest Union-Find} decoder, as it does not maintain a dynamic forest. We dub the Static-Forest Union-Find decoder with weighted growth applied via the bucket sort method introduced in this section the \emph{Static-Forest Bucket Union-Find} decoder. The pseudo-code for this decoder is presented in Algorithm \ref{algo:sfbuf}. We dub the Union-Find decoder with dynamically maintained trees the \emph{Dynamic-Forest Union-Find} decoder. The pseudo-code for this decoder is presented in Algorithm \ref{algo:dfuf}. The decoder that combines both methods is dubbed the \emph{Dynamic-Forest Bucket Union-Find} decoder, and its pseudo-code is presented in Algorithm \ref{algo:dfbuf}. As we compare the performance of these implementations in the next section, we will be using abbreviations according to Table \ref{tab:uftable}.
\begin{table}[htb]
  \centering
  \begin{tabularx}{\textwidth} { | R{0.6} || C{1.2} | C{1.2} | }
    ...
    \hhline{|=||=|=|}
    Static forest & SUF decoder (Alg. \ref{algo:uf}) & SBUF decoder (Alg. \ref{algo:sfbuf})\\
    \hline
    Dynamic forest & DUF decoder (Alg. \ref{algo:dfuf}) & DBUF decoder (Alg. \ref{algo:dfbuf})\\
    \hline
  \end{tabularx}
...
\begin{table}[htb]
  ...
  \begin{tabularx}{\textwidth} { | L{.7} | R{.7} || C{1.8} | C{.5} | C{1.8} | C{.5} | }
    ...
    \multirow{4}*{Toric}    & SUF & $0.09716 \pm 0.00003$ & $0.7336$ & $ 0.02531 \pm 0.00005$ & $0.9239$ \\
                            ...
                            \cline{2-6}
                            & DBUF & $0.10009 \pm 0.00002$ & $0.7279$ & $0.02685 \pm 0.00001$ & $0.9208$ \\
    \hhline{|=|=||=|=|=|=|}
    \multirow{4}{*}{Planar} & SUF & $0.09782 \pm 0.00005$ & $0.8578$ & $0.02570 \pm 0.00003$ & $0.9410$\\
                            ...
                            \cline{2-6}
                            & DBUF & $0.09994 \pm 0.00002$ & $0.8460$ & $0.02660 \pm 0.00001$ & $0.9322$\\
                            \hline
  \end{tabularx}
...
\begin{proposition}
  There is a intuition that a decreased weight is to some extend a heuristic for an increased threshold. 
\end{proposition}
...
\todo[inline]{running time comparison}