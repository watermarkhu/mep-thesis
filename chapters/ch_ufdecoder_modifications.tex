...
\begin{figure}
  ...
  \begin{tikzpicture}[node distance=1cm, on grid]
    ...
    \foreach \i in {0,1,2}{
        \node (a\i) [above = \i cm of a] {$\pre{k}\nset^o_\i$};
        \draw[l1] (a\i) circle[radius=.45cm];
    }
    ...
    \draw[l1] (cc2) -- +(0, 2) arc (0:180:0.6) -- +(0, -2) arc (180:360:0.6) -- cycle;
    \node at (c1) {$\pre{k+1}\nset^o$};
    ...
    \node (cb) at ($(c)!0.5!(b)$) {}; \node [below = 0.4cm of cb] {$f_1$};
    \node [below = 0.4cm of b1t] {$f_2$};
    ...
    \draw[l1, ->] (u1l) -- (u1r) node[midway,above] {union};
    \draw[l1, ->] (u2l) -- (u2r) node[midway,above] {union};
