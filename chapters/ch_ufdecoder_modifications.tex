...
\end{eqnarray*} 
If we were to consider some nodes $n_4, n_5...$ as well, we would find a trend in which the delay of odd node $n_{2i+1}$ would have the positive addition of $\epsilon_{2i}$ in its delay value, and the substraction of $\epsilon_{2i}$ for an even node $n_{2i}$. Thus we can generalize the delay calculation as the following:
...
Using equation \ref{eq:1ddelay}, we can calculate all the relative delays in the 1D tree by traversing the node tree just once from left to right. The initial delay $n_0.d$ can be set at any arbitrary value, as there may be some odd node $n_i$ in the tree that has $n_i.d < n_0.d$. For each node set, we therefore need to store the \emph{minimal delay value}. This value can be stored at the cluster object $C.md$. To grow a cluster, we first calculate its node delays. Then, in each round of growth, we traverse the node tree to find nodes where $n.w = n.d - C.md$, where $n.w$ is the number of growth iterations a node has \emph{waited}. For all nodes that do not satisfy, we add 1 to its waited value. This generates the following pseudocode. 
...
Consider again our example cluster $C_e$ from the previous section. For its node tree $\nset$, let us take $n_1$ as the root of the node tree, $n_2$ is its child and $n_3$ a grandchild. Take $NC_i$ to be the number of children nodes of some node $n_i$ in $\nset$, then we find $NC_1 = 2$, $NC_2 = 1$ and $NC_2 = 0$. We find that all delayed nodes are nodes that have odd parity in its children count. 
...
\subsection{Node delay}\label{sec:nodedelay}
