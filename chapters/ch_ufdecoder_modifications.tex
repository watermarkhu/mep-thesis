...
\begin{figure}
 ...
 \begin{tikzpicture}
   ...
   \path[pattern=north east lines , pattern color=cyan] (2,1) arc (90:450:1);
...
\begin{figure}[h]
  \centering
  \tikzstyle{nodea}=[node, minimum size=20]
  \tikzstyle{nodeb}=[nodel, solid, fill=white!70!black]
  \begin{tikzpicture}
    \node at (-1,1) {$\nset_{1D}$};
    \draw[l1] (0,0) node[nodea] (n1) {$n_0$} -- ++(2,0) node[nodea] (n2) {$n_1$} -- ++(2,0) node[nodea] (n3) {$n_2$} -- ++(2,0) node[nodeb] (n4) {};
    \draw[l1, dotted] (n4) -- ++(1,0) node[nodeb] (n5) {} -- ++(1,0) node[nodeb] (n6) {} -- ++(1,0) node[nodeb] (n7) {};
