...
\end{theorem}
\todo[inline]{this should not be a theorem}
...
\todo[inline]{replace root superscript r with lowerscript r}
...
To grow a cluster represented by a node set $\nset$, we have to traverse the entire set from root to stem to iterate over each boundary list that are stored at the roots. Let's call the total number of times any node is traversed by \codefunc{GrowNode} over $N_{node}$. This is not equal to number of nodes $N_n$ as each node can be traversed many times during growth.
