...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=1.2]
    ...
    \node[nodel, fill=white!70!mblue] at (0) (v0) {\footnotesize$v_1$};
    \node[nodel] at (-1,0) (v0l) {};
    \node[nodel] at (0,1) (v0u) {};
    \node[nodel] at (0,-1) (v0d) {};

    \node[nodel, fill=white!70!morange] at (1) (v1) {\footnotesize$v_2$};
    \node[nodel] at (1,0) (v1l) {\footnotesize$v_{12}$};
    \node[nodel] at (2,1) (v1u) {};
    \node[nodel] at (2,-1) (v1d) {};

    \node[nodel, fill=white!70!mgreen] at (2) (v2) {\footnotesize$v_3$};
    ...
    \node at (7,1) {$\nset_e$};
    \node[nodel, pattern=horizontal lines, pattern color=mblue!50!white] at (7,0) (n0) {\footnotesize$s_1$};
    \node[nodel, pattern=horizontal lines, pattern color=mblue!50!white, line width=0] at (8,0) {};
    \node[nodel, pattern=vertical lines,   pattern color=morange!50!white] at (8,0) (j01) {\footnotesize$l_{12}$};
    \node[nodel, pattern=vertical lines,   pattern color=morange!50!white] at (9,0) (n1) {\footnotesize$s_2$};
    \node[nodel, pattern=vertical lines,   pattern color=morange!50!white, line width=0] at (10,0) {};
    \node[nodel, pattern=horizontal lines, pattern color=mgreen!50!white] at (10,0) (j12) {\footnotesize$l_{23}$};
    \node[nodel, pattern=horizontal lines, pattern color=mgreen!50!white] at (11,0) (n2) {\footnotesize$s_3$};
...
\begin{definition}\label{def:nodedelay}
  Let the \emph{node delay} $n.d$ be the the difference in potential matching weight of a node $n$ and its root node $n_r$.
\end{definition}

\begin{definition}\label{def:nodeparity}
  Let the \emph{node parity} $n.p$ be an indicator for whether the a node $n_\beta$ has a larger delay compared to its parent $n.\alpha$; if $n_\beta.p=0$ then $n_\beta.p < n_\alpha.p$, and if $n_\beta.p=1$ then $n_\beta.p > n_\alpha.p$. 
\end{definition}
...
\begin{theorem}\label{the:delayequation}
  ...
  \begin{equation}\label{eq:nodeparity0}
    n_\beta.p =
    \begin{cases}
      0, & \mbox{if $n_\beta$ has no children}  \\
      \big( \sum_{j} 1-n_{\gamma,j}.p \big ) \bmod 2 \hspace{.2cm} | \hspace{.2cm}  n_\gamma \mbox{ child of } n_\beta, & n_\beta \equiv s_\beta \\
      1 - \big( \sum_{j} 1-n_{\gamma,j}.p \big ) \bmod 2 \hspace{.2cm} | \hspace{.2cm} n_\gamma \mbox{ child of } n_\beta, & n_\beta \equiv l_\beta.
    \end{cases}
  \end{equation}
  The node delay of a node $n_\beta$ is only dependent on its own attributes and its parent $n_\alpha$:
  \begin{multline}\label{eq:delayequation0}
    n_\beta.d = n_\alpha.d + \Bigg \lceil 2f_{eq} \bigg(\ceil{\frac{n_\beta.r}{2}} - \ceil{\frac{n_\alpha.r}{2}} - (-1)^{n_\beta.p}(n_\alpha,n_\beta)\bigg) - (n_\beta.r + n_\alpha.r)\bmod 2 \Bigg \rceil \\ 
    | \hspace{.3cm} n_r.d = 0, \hspace{.2cm} n_\beta \mbox{ child of } n_\alpha, \hspace{.2cm} f_{eq} \in [0, 1],
  \end{multline}
  where $n.r$ denotes the node radius (Definition \ref{def:bloom}) and $f_{eq}$ is an optimization parameter.
\end{theorem}


We will prove Theorem \ref{the:delayequation} throughout the following sections. In Section \ref{sec:1dnodetree}, we introduce the concept of node delays and parities through an example of a one-dimensional node tree. In Sections \ref{sec:nodetreeparity}, \ref{sec:nodetreedelay} and \ref{sec:linkparitydelay}, the same concept is applied to two-dimensional node trees, which is the dimension of the surface code that applies to realistic node trees. In Section \ref{sec:bbstate}, introduce the concept of the \emph{equilibrium} state of a node tree that optimizes the minimal weight behavior through the $f_{eq}$ parameter. Finally, the pseudo-codes for the calculation of node delays are listed in Section \ref{sec:pdccalc}.

\subsection{1D node tree}\label{sec:1dnodetree}
...
\subsection{Node tree parity}\label{sec:nodetreeparity}
...
\subsection{Node tree delay}\label{sec:nodetreedelay}
...
Up until now, the existence of linking-nodes has been neglected in the node parity and delays calculations. But as linking-nodes have the same properties as syndrome-nodes, linking-nodes can be delayed as well, and they must be included in the parity and delay calculations. Furthermore, without linking-nodes, lemma \ref{lem:anynoderoot} cannot be true for a node set $\nset$ for all nodes $n \in \nset$ for the same set of edges $\{\epsilon\}$.
\tikzstyle{linking}=[diamond, inner sep=1]
\begin{figure}
  \centering
  \begin{tikzpicture}
    ...
    \node[left=1 of 0] {$\nset_e$};
      \begin{scope}[shift={(6,0)}]
        \node[even] at (0,5) (0) {$\sigma_0$};
        \node[odd,linking] at (0,4) (1) {$j_1$};
        \node[odd] at (0,3) (2) {$\sigma_2$};
        \node[even,linking] at (0,2) (3) {$j_3$};
        \node[even] at (0,1) (4) {$\sigma_4$};
        \draw[l1] (0) -- (1) -- (2) -- (3) -- (4);
        \node[left=1 of 0] {$\nset_e'$};
      \end{scope}
  \end{tikzpicture}
  \caption{Two example node sets $\nset_e$ and $\nset_e'$ each containing 5 nodes, where syndrome-nodes $\sigma_i$ are circles and linking-nodes $j_i$ are diamonds. Their appropriate parities are calculated where an even parity correspond to continuous lines and odd to dashed lines. }\label{fig:linkingparity}
\end{figure}
As a linking-node is also allowed to bloom, similarly to a syndrome-node, equation \eqref{eq:2ddelay} still holds for linking-nodes. However, the parity of a linking-node is calculated differently. Consider an example node set $\nset_e$ with 5 syndrome-nodes $\{\sigma_1,...,\sigma_5\}$ lined up linearly with distance 1 between them and $n_r = \sigma_1$. Let us drop the $n.s, n.g$ components of the delay in equation \eqref{eq:2ddelay} as we are now only interested in the parity component $(-1)^{n_\beta.p-1+1}(n_\alpha, n_\beta)$. The parity of $\sigma_4$ is odd, therefore
\begin{equation*}
  \sigma_4.d = \sigma_3.d + 2(\sigma_3, \sigma_4).
\end{equation*}

Consider now a second example node set $\nset_e'$ with 3 syndrome-nodes and 2 linking-nodes $\{\sigma_1, j_2, \sigma_3, j_4, \sigma_5\}$. The PMW's for $\sigma_3$ and $j_4$ are $(\sigma_1, j_2) + (j_4, \sigma_5)$ and  $(\sigma_1, j_2) + (\sigma_3, j_4) + (j_4, \sigma_5)$, respectively, where the delay in $j_4$ is now
\begin{equation*}
  j_4.d = \sigma_3.d - 2(\sigma_3, j_4).
\end{equation*}
The edge component of the delay calculation now has an opposite sign. This flip in sign is due to a flip in node parity for linking-nodes compared to syndrome-nodes. As a result, we can generalize the parity calculation of equation \eqref{eq:nodeparitypart} for node sets including both syndrome-nodes and linking-nodes.
...
To put this into perspective of lemma \ref{lem:nodeCalcParity}, the parity of a syndrome-node is the number of children \emph{syndrome}-nodes. The parity of a linking-node is 1 minus the number of children syndrome-nodes. From here, our definitions of parity and delay calculation stay unchanged; the parities can to be calculated by a reversed DFS of the node tree from the root with equation \eqref{eq:nodeparity}, and the delays by a second DFS with equation \eqref{eq:2ddelay}.

\begin{lemma}\label{lem:nodecalc_linking}
  The node parity in a syndrome-node $\sigma.p$ is the number of children syndrome-nodes $\sigma_\gamma$ modulo 2. The node parity in linking-node $j.p$ is 1 minus the above definition.
\end{lemma}
